...
sig_all_env$origin270 <- ifelse(grepl("270", sig_all_env$origin, fixed = FALSE),
                                 "270","others")
sig_all_env$origin270 <- factor(sig_all_env$origin270, levels = c("others","270"))
...
gwas_res_all$origin270 <- ifelse(grepl("270", gwas_res_all$origin, fixed = FALSE),
                                 "270","others")
gwas_res_all$origin270 <- factor(gwas_res_all$origin270, levels = c("others","270"))

p.all <- ggplot(data = gwas_res_all, aes(x=beta.nodall, y=beta.shootall)) +
  geom_rect(aes(xmin=0, xmax=Inf, ymin=-Inf, ymax=0), fill="grey90", alpha=0.5) + 
  geom_point(size = 2, alpha = 0.2) + 
  geom_smooth(method=lm, se=FALSE, colour="black") + 
  geom_point(aes(x=beta.nodall, y=beta.shootall, color=origin270), 
             data=sig_all_env, size = 4, fill="black") + 
  geom_hline(aes(yintercept=0), linetype="dashed")+
  geom_vline(aes(xintercept=0), linetype="dashed")+
