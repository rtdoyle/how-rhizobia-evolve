...
First, need to format your phenotypes file:
...
load("./Result_outputs/stand_means_for_GEMMA.Rdata") ## load stand_means.w

# add in vcf sample names
isolate_info <- read_csv("./pheno_SI_exp_files/isolate_info.csv")
...
load(file = "./Result_outputs/GEMMA_363_sum.Rdata") ## load GEMMA.vars.f
...
# save file
save(GEMMA_vars_sig, file = "./Result_outputs/GEMMA_363_sig.Rdata")
```

## Figure 3: Regress shoot and nod beta scores in each environment
...
# load df
load(file = "./Result_outputs/GEMMA_363_sig.Rdata") # loads GEMMA_vars_sig
...
save(sig_all_env, file = "./Result_outputs/sig_all_env.Rdata")
...
save(fig3_plots, file = "./Result_outputs/fig3_plots.Rdata")
...
save_plot("./Figures/Fig3_gwas.pdf", fig3, dpi= 1000,
...
## Summarize variants (combine dataframes generated in Fig. 3)

```{r sum_vars}
load(file = "./Result_outputs/sig_all_env.Rdata") # loads sig_all_env
load(file = "./Result_outputs/fig3_plots.Rdata") # fig3_plots
...
write.csv(sig_ps_sum, "./Result_outputs/sig_ps_sum.csv", row.names = FALSE)
save(sig_ps_sum, file = "./Result_outputs/sig_ps_sum.Rdata")
...
# save wide_gene df
write.csv(sig_gene_sum, "./Result_outputs/sig_gene_sum.csv", row.names = FALSE)
```

## Summarize long version of GEMMA_vars_sig

long: Each row is one sig effect
wide_ps: each row is one position
wide_gene: each row is one gene
...
rownames(GEMMA_vars_sig) <- GEMMA_vars_sig[,1] ## name rows by position

# use gather to convert wide to long
gwas_res_long.1 <- GEMMA_vars_sig %>% 
  gather(betas, scores, beta.shoot267:beta.nodall)

gwas_res_long.2 <- GEMMA_vars_sig %>% 
  gather(betas, sig, beta.shoot267.FIsig:beta.nodall.FIsig)
 
# merge into one, getting rid of redundant cols
gwas_res_long <- cbind(gwas_res_long.1[,-c(26:37)],gwas_res_long.2[,39])

# Rename cols
colnames(gwas_res_long)[colnames(gwas_res_long)=="betas"] <- "effect"
colnames(gwas_res_long)[colnames(gwas_res_long)=="gwas_res_long.2[, 39]"] <- "perms"

save(gwas_res_long, file = "./Result_outputs/gwas_long_all.Rdata")

# filter any non-significant vars
gwas_res_long_sig <- gwas_res_long %>%
  filter(perms != "FALSE") %>%
  droplevels()

# Rename effect 
gwas_res_long.rn <- gwas_res_long_sig %>% 
  mutate(effect = factor(effect, levels = c("beta.shootall", "beta.shoot270", "beta.shoot276", 
                                            "beta.shoot279", "beta.shoot313", "beta.shoot267",
                                            "beta.nodall", "beta.nod270", "beta.nod276", 
                                            "beta.nod279", "beta.nod313", "beta.nod267"),
                              labels = c("Shoot.All", "Shoot.270","Shoot.276",
                                         "Shoot.279","Shoot.313","Shoot.267",
                                         "Nod.All", "Nod.270","Nod.276",
                                         "Nod.279","Nod.313","Nod.267"))) 

# split into trait and env
gwas_res_long.s <- separate(data = gwas_res_long.rn, 
                            col = effect, 
                            into = c("trait", "env"), sep = "\\.")

# determine whether effects are local or not
gwas_res_long.l <- gwas_res_long.s %>%
   mutate(Local = 
           str_detect(as.character(gwas_res_long.s$origin), 
                      as.character(gwas_res_long.s$env)) 
       )

gwas_res_long.sl <- gwas_res_long.l %>% 
  mutate(Local = factor(Local, levels = c("FALSE", "TRUE"),
                              labels = c("No", "Yes"))) 

# get rid of unwanted cols
gwas_res_long.f <- gwas_res_long.sl[ , -which(names(gwas_res_long.sl) %in% c("perms"))]

# add minor allele effect (C = pos, D = neg)
gwas_res_long.f$ma_effect <- ifelse(gwas_res_long.f$scores > 0, "C", "D")

# save long df
write.csv(gwas_res_long.f, "./Result_outputs/gwas_res_long_24Nov2019.csv", row.names = FALSE)

# get total numbers of each category
gwas_res_long.sum <- gwas_res_long.f %>%
  filter(Novel == "Yes") %>%
  group_by(Local, ma_effect) %>%
  summarize(count = n())

ggplot(gwas_res_long.sum, aes(x = Local, y = count, fill = ma_effect)) +
  geom_bar(stat="identity", position=position_dodge())

# Summarize at the position-level
gwas_res_long.ps_sum <- gwas_res_long.f %>%
  mutate(effect_cat = paste(Local, ma_effect, sep = '-'),
         effect_cat2 = paste(env, Local, ma_effect, sep = '-')) %>%
  group_by(ps, env, effect_cat) %>%
  mutate(no_effects = n(),
         effect_cat_no = paste0(effect_cat, "(", no_effects, ")")) %>%
  ungroup %>%
  spread(key = env, value = effect_cat_no) %>%
  rename(
    "L270" = '270',
    "L276" = '276',
    "L279" = '279',
  ...
  group_by(CHROM, ps, gene, gene_ID, ncbi_func, RefSeq_ID, protein_ID, 
           var_type, origin, Novel) %>%
  summarize(traits = paste(unique(trait), collapse = ", "),
    effect_cats = paste(unique(effect_cat2), collapse = ", "),
    no_effects = n(),
...
# categorize the vars into broad, local and mismatch
gwas_res_long.ps_sum$cat <- ifelse(grepl(", ", gwas_res_long.ps_sum$effect_cats) | 
                                     grepl("All", gwas_res_long.ps_sum$effect_cats), "Broad",
                                   ifelse(grepl("Yes", gwas_res_long.ps_sum$effect_cats),
                                          "Local", "Mismatch"))
                                   
# rename traits
gwas_res_long.ps_sum$traits <- ifelse(gwas_res_long.ps_sum$traits == "Shoot", "Quality",
                                      ifelse(gwas_res_long.ps_sum$traits == "Nod",
                                             "Fitness","Both"))

gwas_res_long.ps_sum <- gwas_res_long.ps_sum %>%
  arrange(cat, CHROM, ps)

# add effect cat to long
gwas_res_long.fec <- merge(gwas_res_long.f, 
                           gwas_res_long.ps_sum[,c("ps","cat","traits")], 
                           by = "ps", all.x = TRUE)

# save wide_ps df
write.csv(gwas_res_long.ps_sum, "./Result_outputs/gwas_res_wide_ps.csv", row.names = FALSE)
save(gwas_res_long.ps_sum, file = "./Result_outputs/gwas_ps_sum.Rdata")

# summarize variants
gwas_res_long.ps_sum$effect_dir <- ifelse(gwas_res_long.ps_sum$ave_score > 0,
                                          "Beneficial", "Deleterious")

# Summarize at the gene-level
gwas_res_long.gene_sum <- gwas_res_long.fec %>%
  mutate(effect_cat = paste(Local, ma_effect, sep = '-'),
         effect_cat2 = paste(env, Local, ma_effect, sep = '-')) %>%
  group_by(gene, env, effect_cat) %>% 
  mutate(no_effects = n(),
         effect_cat_no = paste0(effect_cat, "(", no_effects, ")")) %>%
  ungroup %>%
  spread(key = env, value = effect_cat_no) %>%
  ...
  group_by(CHROM, gene, ncbi_func, RefSeq_ID, protein_ID) %>%
  summarize(no_vars = n_distinct(ps),
            min_ps = min(ps), 
            max_ps = max(ps), 
            var_types = paste(unique(var_type), collapse = ", "),
...
# format position range
gwas_res_long.gene_sum$ps_range <- ifelse(gwas_res_long.gene_sum$min_ps == gwas_res_long.gene_sum$max_ps, gwas_res_long.gene_sum$min_ps, 
                                          paste(gwas_res_long.gene_sum$min_ps,
                                                gwas_res_long.gene_sum$max_ps, sep = "-"))

# rename traits
gwas_res_long.gene_sum$traits <- ifelse(gwas_res_long.gene_sum$traits == "Quality", "Quality",
                                      ifelse(gwas_res_long.gene_sum$traits == "Fitness",
                                             "Fitness","Both"))

# save wide_gene df
write.csv(gwas_res_long.gene_sum, "./Result_outputs/gwas_res_wide_gene.csv", row.names = FALSE)
save(gwas_res_long.gene_sum, file = "./Result_outputs/gwas_gene_sum.Rdata")
```